...
function TrendingCards ({item}){
    ...
    return(
        ...
        <div className="card" id="main-card">
            <img onClick={() => history.push(`/detail/${collection[0].address}/${item.nft_id}`)} src={`${process.env.REACT_APP_BASE_URL}${item.image_local}`} id="card-img" style={{minHeight:'250px',maxHeight:'250px'}} className="card-img-top" alt="..."></img>
            <div className="card-body">
                <div className="d-flex justify-content-between">
                    <span className="card-title h5 pt-1 text-white" id="card-title-font">{item.name}</span>
                    <span className="card-title" id="heart-background"><i id="heart" onClick={like} className="fas fa-heart" style={{color:authedUser.authedUser.address ? (item.likedBy.includes(authedUser.authedUser._id) ? 'red' : '#162e4a') : '#162e4a'}}></i></span>
                </div>

    ...
    try{
        ...
        if (collection.address === process.env.REACT_APP_EMPIRE_TOKEN){
          ...
          minted = await NFTMinter.methods.createItem(item.token_uri).send({
            ...
            console.log("Other Collection Minting", collection.address, item.token_uri);
            let minterNFT = new web3.eth.Contract(ERC721ABI, collection.address);
            console.log(minterNFT);
            minted = await minterNFT.methods.safeMint(item.token_uri).send({
                from: authedUser.authedUser.address
            });
        }
        console.log(minted);
            ...
            {item.minted ? (
              <>
                ...
                {item.status === "2" && ( // on auction
                  ...
                  <p className="mb-1 text-left  text-white" id="bid-color"> <span  onClick={() => endAuction()} style={{textDecoration: 'underline', cursor: 'pointer', position: 'absolute'}} id="blue-color"> End Auction</span></p>
                </>
                )}
              </>
            ) : (
